...
import {
  computeKeyCallDashboard,
  getKeyDashboardOptions,
  getProjectRechargeBalance,
  type KeyCallDashboardQuery,
  type KeyCallDashboardResult,
  type KeyCallDashboardStats,
  type KeyScopeType,
  type ProjectRechargeBalance,
} from '../components/keyCallDashboardStats';
import { formatCny, formatTokens } from '../components/mockData';
...
const keyOptions = getKeyDashboardOptions();
const defaultKeyId = keyOptions[0]?.value ?? '';

type AppliedQuery = KeyCallDashboardQuery & {
  queriedAt: Date | null;
};
...
function getKeyLabel(keyId: string): string {
  return keyOptions.find((item) => item.value === keyId)?.label ?? keyId;
}

function getKeyCode(keyId: string): string {
  return keyOptions.find((item) => item.value === keyId)?.desc ?? '';
}
...
function buildStatItems(stats: KeyCallDashboardStats): StatItem[] {
  return [
    {
      key: 'tokens',
      label: '总调用 Token 数',
      tip: '维度为「全部」时统计全部已启用密钥；选择具体密钥时统计单项 Token。整数展示。',
    ...
    },
    {
      key: 'cost',
      label: '总 Token 消费金额',
      tip: '运营管理端模型客户单价 × Token 数量，金额遵循全局规则。',
...
const DEFAULT_QUERY: KeyCallDashboardQuery = {
  keyScope: 'all',
  keyId: defaultKeyId,
  dateRange: null,
};

export default function KeyCallDashboardPage() {
  ...
  const [dashboardResult, setDashboardResult] = useState<KeyCallDashboardResult>(() =>
    computeKeyCallDashboard(DEFAULT_QUERY),
  );
  ...
  const keyDimensionOptions = useMemo(
    () => [{ value: 'all', label: '全部' }, ...keyOptions],
    [],
  );

  const statItems = useMemo(
    () => buildStatItems(dashboardResult.stats),
  ...
  const scopeLabel = useMemo(() => {
    if (appliedQuery.keyScope === 'all') {
      return {
        name: '全部',
        code: '',
        range: formatDateRange(appliedQuery.dateRange),
      };
    }
    return {
      name: getKeyLabel(appliedQuery.keyId),
      code: getKeyCode(appliedQuery.keyId),
      range: formatDateRange(appliedQuery.dateRange),
    };
  }, [appliedQuery]);

  const toDashboardQuery = useCallback(
    (values: { keyDimension?: string; dateRange?: [Dayjs, Dayjs] | null }): KeyCallDashboardQuery => {
      const keyDimension = values.keyDimension ?? 'all';
      return {
        keyScope: keyDimension === 'all' ? 'all' : 'single',
        keyId: keyDimension === 'all' ? defaultKeyId : keyDimension,
  ...
  const applyQuery = useCallback((values: KeyCallDashboardQuery) => {
    setLoading(true);
    setChartLoading(true);
    window.setTimeout(() => {
      const result = computeKeyCallDashboard(values);
      setAppliedQuery({
        ...values,
        queriedAt: new Date(),
      });
  ...
  const hasQueryTarget = appliedQuery.keyScope === 'all' || Boolean(appliedQuery.keyId);
  ...
  return (
    <div className="dev-platform-page">
      <PageHeader
        title="密钥调用数据看板"
        description="统计密钥维度下的调用及消费情况；指标卡片与趋势图随查询条件联动刷新。金额遵循全局规则，Token 为整数。"
      />
      ...
      >
        ...
        <div className="dashboard-filter-compact">
          <Form
            ...
            initialValues={{ keyDimension: 'all' }}
          >
            <div className="dashboard-filter-compact-row">
              <Form.Item
                ...
                rules={[{ required: true, message: '请选择密钥维度' }]}
              >
                <Select
                  placeholder="请选择"
                  showSearch
                  optionFilterProp="label"
                  options={keyDimensionOptions}
                  optionRender={(option) =>
                    option.value === 'all' ? (
      ...
      <div className="dashboard-charts">
        <Card bordered={false} className="page-card chart-card">
          ...
          <Spin spinning={chartLoading}>
            {!hasQueryTarget ? (
              <Empty className="chart-empty" description="请选择密钥范围后查询" />
            ) : tokenTrend.length === 0 ? (
